...
class DataCollector:
    ...
    def __init__(self):
        ...
        # 2) Dimensions
        self.Nstates = self.env.plant.num_multibody_states()  # 18 = 9 q + 9 v
        self.nq      = self.env.plant.num_positions()         # 9
        self.udim    = self.env.controller_plant.num_actuators()  # 7

        # 3) Actuation mapping B: generalized_forces = B @ u
        self.B = self.env.plant.MakeActuationMatrix()         # shape (9,7)
    ...
    def step(self, x, u):
        ...
        tau_act = self.B.dot(u)            # → R^9

        M   = plant.CalcMassMatrix(ctx)    # 9×9
        Cv  = plant.CalcBiasTerm(ctx)      #  9
        tauG = plant.CalcGravityGeneralizedForces(ctx)  # 9

        qdd = np.linalg.solve(M, tau_act - Cv - tauG)   # → R^9
        x_dot = np.concatenate([x[self.nq:], qdd])     # 18
...
if __name__ == "__main__":
    ...
    np.save("G1_cartpole_data.npy", data)
